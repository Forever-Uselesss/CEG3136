...
void titleScreen(void) {
  ...
  if ((GPIO_PortInput(GPIOX) & (1 << StartButton.bit)) && TimePassed(lastPress) > DEBOUNCE_TIME) {
  ...
  if (!ledOn && TimePassed(lastFlash) > OFF_TIME) {
    ...
    if (di == true) {
      DisplayPrint(1, "HIGH SCORE: %lu", HIGH_SCORE);
    } else {
...
void Task_Game(void) {
  ...
  case DISPLAY:
    if (TimePassed(now) > ON_TIME) {
      DisplayPrint(1, "Level %lu", score + 1);
      displaySequence();
    }
    break;
  ...
  case GAMEOVER:
    if (displaying) {
      ...
      DisplayPrint(1, "FINAL SCORE: %lu", score);
      UpdateDisplay();
      displaying = false;
    }
    if (score > HIGH_SCORE) {
