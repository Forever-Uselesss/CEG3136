...
static const EnvWrite_t txOvsp[2] = {{0x072, 0x01}, {0x74, 0x40}}; // Page 1
...
static const EnvWrite_t txParHAddr = {0xE2 | READ}; // Page 0
...
static double CalcTemperature() {
  ...
  temp_adc = ((uint32_t)tempData[0] << 12) | ((uint32_t)tempData[1] << 4) | ((uint32_t)tempData[2] >> 4);

  // Refer to datasheet 3.3.1 and Table 11
  var1 = (((double)temp_adc / 16384.0) - ((double)par_t[0] / 1024.0)) * (double)par_t[1];
  var2 = ((((double)temp_adc / 131072.0) - ((double)par_t[0] / 8192.0)) * (((double)temp_adc / 131072.0) - ((double)par_t[0] / 8192.0))) *
         ((double)par_t[2] * 16.0);
  t_fine = var1 + var2;
...
static double CalcHumidity(double temp_comp) {
  ...
  hum_adc = ((uint16_t)humidityData[0] << 8) | ((uint16_t)humidityData[1]);

  var1 = hum_adc - (((double)par_h[0] * 16.0) + (((double)par_h[2] / 2.0) * temp_comp));
  var2 = var1 * (((double)par_h[1] / 262144.0) *
                 (1.0 + (((double)par_h[3] / 16384.0) * temp_comp) + (((double)par_h[4] / 1048576.0) * temp_comp * temp_comp)));
...
static void ProcessEnvData(void) {
  ...
  DisplayPrint(ENVIRO, 0, "Temp: %.2f C", temperature);
  DisplayPrint(ENVIRO, 1, "Hum:  %.2f %%", humidity);
}
