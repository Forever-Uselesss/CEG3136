...
void I2C_Enable(I2C_Bus_t bus) {
  ...
  RCC->APB1ENR1 |= bus.iface == I2C1   ? RCC_APB1ENR1_I2C1EN
                   ...
                   : bus.iface == I2C4 ? RCC_APB1ENR2_I2C4EN
                                       : 0;
  ...
  bus.iface->TIMINGR = 0x20443F5F; // 100kHz @ 48MHz SYSCLK
  bus.iface->CR1 = I2C_CR1_PE;
}
...
void ServiceI2CRequests(void) {
  ...
  if (n == -1) {
    ...
    i2c->CR2 = (q->addr & 0xFE) | I2C_READ << I2C_CR2_RD_WRN_Pos | q->size << I2C_CR2_NBYTES_Pos | q->stop << I2C_CR2_AUTOEND_Pos | I2C_CR2_START;
  } else if (n < q->size) {
